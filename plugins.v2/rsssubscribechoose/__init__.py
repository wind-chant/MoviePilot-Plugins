...
from app.schemas import ExistMediaInfo, ServiceInfo
...
class RssSubscribeChoose(_PluginBase):
    # 插件名称
    plugin_name = "自定义订阅(下载器可选)"
    # 插件描述
                    ...
                    'content': [
                        {
                            ...
                            'events': {
                                'click': {
                                    'api': 'plugin/RssSubscribeChoose/delete_history',
                                    'method': 'get',
                                    'params': {
                                        'key': title,
            ...
            for result in results:
                try:
                    ...
                    if self._action == "download":
                        # 如果指定了下载器，使用指定的下载器
                        if self._downloader and self.downloader:
                            # 获取下载链接
                            torrent_content = torrentinfo.enclosure
                            ...
                            # 如果种子地址不是磁力地址，则请求种子到内存再传入下载器
                            if not torrent_content.startswith("magnet"):
                                response = RequestUtils(proxies=proxies).get_res(url=torrent_content)
                            ...
                            if downloader_helper.is_downloader("qbittorrent", service=self.service_info):
                                ...
                                if torrent_content:
                                    state = self.downloader.add_torrent(
